...
class RunRequest(BaseModel):
    ...
    viewport: Optional[str] = "desktop"  # desktop | mobile

class AuditRequest(BaseModel):
    target: Optional[str] = "full"  # full | booking | emails | chatbot | finance
...
async def run_scenario(req: RunRequest, x_agent_token: str = Header(...)):
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": str(e), "scenario": req.scenario})
    finally:
        await agent.close()
...
async def full_audit(req: AuditRequest, x_agent_token: str = Header(...)):
    ...
    scenarios = {
        ...
        "emails": ["email_confirmation_check"],
